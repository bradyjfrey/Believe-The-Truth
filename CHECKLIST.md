...
- [ ] **Jumping — fix applied, needs test.** Was: only Girl A could jump (a little); Momotaro/Rokurokubi/Otohime couldn't at all — the dressed rigs shipped with inconsistent Humanoid jump settings. Fix: `Bootstrap.applyCharacterStats` forces a known jump state at spawn — `Constants.Jump.Power` (50) for Momotaro/Otohime/GirlA, with a per-character override of **0 for Rokurokubi (she floats → jumping fully disabled)**. **TEST:** Momotaro/Otohime/GirlA do a small map-contained hop; Rokurokubi can't jump at all. Tune `Constants.Jump.Power` (default) or add `Constants.Jump.PerCharacter` entries.
