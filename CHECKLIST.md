...
- [ ] **Momotaro's companion bird (Hawk)** — wired into spawn, **needs test.** The `Hawk` model lives in `ReplicatedStorage.Companions` with its own behavior script (flap + ride beside a target + cancel the baked flight-circle). `Momotaro.StartPassives` now clones it and hands it a `FollowTarget` ObjectValue = the player character (it was looking for "Kijiro" before, hence the placeholder block). The bird's script reads `FollowTarget` instead of name-searching, and parts are now anchored + non-colliding. **TEST:** spawn as Momotaro → Hawk should flap beside his shoulder and follow him. Tune offset/facing via `RIGHT_OFFSET`/`UP_OFFSET`/`FORWARD_OFFSET`/`FACING_YAW` at the top of the bird's script.
