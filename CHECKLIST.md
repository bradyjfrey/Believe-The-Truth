...
- [ ] **Otohime — 2nd survivor (Warden).** Model is already built in the workspace; **first step = check her welds** (inspector report like we did for Momotaro, before any wiring). Then: add her to `RoundService.DRESSED_ENABLED`, give Wardens a **survivor select screen** (mirror the Yokai picker — same `CharacterPickerUI` / `CharacterPicker` remote; today every Warden is hardcoded to Momotaro at `RoundService` ~L319). **Survivor-screen spec:** swap the killer's red hover/select highlight for **light blue**; keep a **"COMING SOON" locked slot** (same as the Yokai screen). Her two effects (Healing Pulse, Dark Moon) listed under Effects below.
